  ...
  <details class="dropdown dropdown-end" ref="dropdown">
    <summary class="btn btn-ghost btn-circle m-0 p-0 font-light text-primary-content">
      <div class="indicator">
        ...
        <span
          v-if="messageCount"
          class="badge badge-xs font-bold indicator-item text-base-100"
          :class="`bg-${lastMessageType}`"
        >
          {{ messageCount }}
        </span>
      </div>
    ...
    <ul
      tabindex="0"
      class="mt-2 p-2 shadow menu dropdown-content z-[1] bg-base-100 rounded-box w-72"
      v-if="messageCount"
    >
      <li
        v-for="(message, index) in store.messages.slice(0, 5)"
        class="p-1"
      ...
      <li>
        <a href="#" class="block bg-neutral text-neutral-content text-center font-bold py-2"
...
onClickOutside(dropdown, (event) => {
  if (dropdown.value.open) dropdown.value.removeAttribute('open')
})
