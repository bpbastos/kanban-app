<template>
  <div class="flex justify-stretch items-center space-x-1 m-2" v-show="totalTasks > 0">
    <div
      class="badge font-semibold"
      :class="totalTasks == totalTasksDone ? 'badge-success' : 'badge-info'"
    >
      ...
      <p class="tracking-wider">{{ totalTasksDone }}/{{ totalTasks }}</p>
    </div>
    <progress
      class="progress shadow-2xl"
      :class="totalTasks == totalTasksDone ? 'progress-success' : 'progress-info'"
