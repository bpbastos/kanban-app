<template>
    <label for="title" class="block overflow-hidden px-3 py-2 w-3/4">
        <span class="text-gray-400 uppercase">Sub Tarefas:</span>
        <ul>
            <li v-for="(st, index) in subTasks"
                ...
                <div class="flex w-2/3">
                    ...
                    <h4 class="ml-2" :class="st.done ? 'line-through' : ''" @click="st.done = !st.done">
                        {{ st.text }}
                    </h4>
                </div>
        ...
        <input type="title" id="title" v-model="newSubTask" @keyup.enter="addSubTask"
            placeholder="Adicionar uma sub tarefa..." class="border-b border-gray-300 outline-none mt-2 w-full" />
    </label>
</template>
